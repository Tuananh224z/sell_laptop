...
import {
  ...
  Percent, Crown, Monitor, BellRing, HeadphonesIcon, Settings
} from 'lucide-react';
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-gray-800 ${collapsed ? 'justify-center' : ''}`}>
        {!collapsed && <span className="font-bold text-white text-lg tracking-tight">Admin TechStore</span>}
      </div>
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full overflow-hidden border border-primary-100 flex items-center justify-center">
                {getAvatar(user?.avatar)
                  ...
                  : <div className="w-full h-full bg-primary-100 text-primary-700 flex items-center justify-center font-bold text-xs uppercase">{adminInitials}</div>
                }
              </div>
