...
import { Search, X, Send, MessageCircle, Image, Loader2, Paperclip, PlayCircle } from 'lucide-react';
...
const AdminSupport = () => {
  ...
  useEffect(() => {
    if (selectedId && socketRef.current) {
      ...
      const handleNewMessage = (msg: Message) => {
        ...
        setTickets(prev => prev.map(t => t._id === selectedId ? { ...t, lastMessageAt: msg.createdAt } : t));
      };
  ...
  const fetchDetail = async (id: string) => {
    try {
      ...
      setTickets(prev => prev.map(t => t._id === id ? { ...t, messages: data.messages } : t));
    } catch (err) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const { data } = await api.patch(`/support/admin/${selectedId}/status`, { status: newStatus });
      setDetail(prev => prev ? { ...prev, status: data.status } : null);
      setTickets(prev => prev.map(t => t._id === selectedId ? { ...t, status: data.status } : t));
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      setReply('');
      setMedia(null);
      setTickets(prev => prev.map(t => t._id === selectedId ? { ...t, lastMessageAt: new Date().toISOString() } : t));
    } catch (err) {
  ...
  const getUnreadCount = (ticket: Ticket) => {
    return ticket.messages.filter(m => m.sender?._id !== user?._id && !m.isRead).length;
  };
  ...
  return (
    <div className="p-6 h-full flex flex-col">
      <div className="mb-5 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">Hỗ trợ khách hàng</h1>
          <p className="text-sm text-gray-500 mt-0.5">Quản lý yêu cầu hỗ trợ trực tuyến</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 flex-1 min-h-0 overflow-hidden">
        {/* Ticket list */}
        <div className={`${selectedId ? 'hidden lg:flex' : 'flex'} lg:col-span-2 flex-col bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden`}>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50 custom-scrollbar">
            {loading ? (
              <div className="p-10 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-primary-500" /></div>
            ) : tickets.length === 0 ? (
              <div className="p-10 text-center text-gray-400 text-sm">Không có yêu cầu nào</div>
            ) : tickets.map(t => {
                ...
                >
                  <div className="flex gap-4">
                    <div className="shrink-0 mt-1">
                      {t.user?.avatar ? (
                        <img src={`${BACKEND}${t.user.avatar}`} className="w-10 h-10 rounded-2xl object-cover shadow-sm border border-white" alt="" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <p className="font-bold text-gray-900 text-sm line-clamp-1">{t.subject}</p>
                        {unread > 0 && (
                      ...
                      <p className="text-[11px] text-gray-500 mb-2 font-medium">
                        {t.user?.name} · <span className="text-gray-400">{new Date(t.createdAt).toLocaleDateString('vi-VN')}</span>
                      </p>
                      <div className="flex gap-2">
                        <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${statusStyle[t.status]}`}>
                          {statusLabel[t.status]}
                        </span>
                        <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${priorityStyle[t.priority]}`}>
        ...
        {/* Conversation */}
        <div className={`${selectedId ? 'flex' : 'hidden lg:flex'} lg:col-span-3 flex-col bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden`}>
          {detail ? (
            <>
              {/* Header */}
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-4 bg-gray-50/30">
                <button onClick={() => setSelectedId(null)} className="lg:hidden p-2 -ml-2 rounded-xl hover:bg-gray-200 text-gray-500"><X className="w-5 h-5" /></button>
                <div className="flex-1 min-w-0">
                  <h3 className="font-extrabold text-gray-900 truncate">{detail.subject}</h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    {detail.user?.avatar ? (
                      <img src={`${BACKEND}${detail.user.avatar}`} className="w-5 h-5 rounded-full object-cover" alt="" />
                    ) : (
                      <div className="w-5 h-5 bg-primary-100 rounded-full flex items-center justify-center text-[8px] font-bold text-primary-700">
                        {detail.user?.name[0]}
                      </div>
                    )}
                    <p className="text-xs text-gray-500 font-medium">{detail.user?.name} <span className="text-gray-300 mx-1">|</span> {detail.user?.email}</p>
                  </div>
                </div>
                <select 
                  value={detail.status} 
              ...
              <div ref={chatRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/20 custom-scrollbar">
                ...
                <div className="flex justify-start">
                  {detail.user?.avatar ? (
                    <img src={`${BACKEND}${detail.user.avatar}`} className="w-8 h-8 rounded-full object-cover shrink-0 mr-3 mt-1 shadow-sm border border-white" alt="" />
                  ) : (
                ...
                {detail.messages.slice(1).map((msg, i) => {
                  ...
                  return (
                    <div key={i} className={`flex ${isStaff ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[80%] ${isStaff ? 'bg-primary-600 text-white rounded-2xl rounded-tr-sm shadow-md shadow-primary-100' : 'bg-white border border-gray-100 text-gray-800 rounded-2xl rounded-tl-sm shadow-sm'} px-5 py-3.5`}>
                        {msg.mediaUrl && (
                        ...
                        <p className={`text-[9px] mt-2 font-bold uppercase tracking-wider ${isStaff ? 'text-primary-200' : 'text-gray-400'}`}>
                          {new Date(msg.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
                          {isStaff && <span className="ml-2 opacity-60">✓✓</span>}
                        </p>
                      </div>
                      {isStaff && (
                        msg.sender?.avatar ? (
                          <img src={`${BACKEND}${msg.sender.avatar}`} className="w-8 h-8 rounded-full object-cover shrink-0 ml-3 mt-1 shadow-sm border border-white" alt="" />
                        ) : (
                          <div className="w-8 h-8 bg-gray-900 text-white rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ml-3 mt-1">AD</div>
                        )
                      )}
                    </div>
                  );
              ...
              <div className="border-t border-gray-100 p-4 bg-white">
                ...
                <p className="text-[10px] text-gray-400 mt-2 px-2 font-medium">Nhấn <strong>Enter</strong> để gửi nhanh, <strong>Shift+Enter</strong> để xuống dòng.</p>
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-200 p-10 bg-gray-50/30">
              ...
              <h4 className="text-gray-900 font-extrabold text-lg">Hộp thư hỗ trợ</h4>
              <p className="text-gray-400 text-sm mt-1 font-medium">Chọn một yêu cầu bên trái để bắt đầu hỗ trợ khách hàng</p>
            </div>
          )}
        </div>
      </div>
