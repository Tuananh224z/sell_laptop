...
import { Search, Eye, ChevronDown, ChevronUp, X, MapPin, Phone, Package, Mail } from 'lucide-react';
...
interface Order {
  ...
  shippingAddress: { name: string; phone: string; address: string; ward: string; district: string; city: string };
...
const OrderDetailModal = ({ order, onClose, onUpdate }: { order: Order; onClose: () => void; onUpdate: (id: string, s: string) => void }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="font-extrabold text-gray-900 text-lg">#{order.orderNumber}</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              {new Date(order.createdAt).toLocaleString('vi-VN')} · {order.paymentMethod.toUpperCase()}
            </p>
          </div>
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          <div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              ...
              <p className="text-sm text-gray-600 flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" />{order.shippingAddress.phone}</p>
              <p className="text-sm text-gray-600 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" />{order.user?.email}</p>
                ...
                <div key={i} className="bg-gray-50 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-500">
                      SL: {item.quantity} {item.variantLabel ? `· ${item.variantLabel}` : ''}
                    </p>
                  </div>
                  <p className="text-sm font-bold text-gray-900 shrink-0">{(item.price * item.quantity).toLocaleString('vi-VN')}₫</p>
                </div>
              ))}
              <div className="flex justify-between items-center px-2 pt-1">
                <span className="text-sm font-bold text-gray-900">Tổng tiền:</span>
                <span className="text-lg font-black text-primary-600">{order.total.toLocaleString('vi-VN')}₫</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
          <button onClick={onClose} className="flex-1 border border-gray-200 rounded-xl py-2.5 text-sm font-semibold text-gray-700 hover:bg-white bg-white transition-colors">Đóng</button>
          <button 
            onClick={handleSave} 
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading ? 'Đang tải...' : `${filtered.length} đơn hàng được tìm thấy`}
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50/50">
              <tr>
                {['Mã đơn', 'Khách hàng', 'Tổng tiền', 'Ngày đặt', 'Thanh toán', 'Trạng thái', 'Thao tác'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-5 py-12 text-center text-gray-400">Đang tải dữ liệu...</td>
                </tr>
              ) : filtered.length > 0 ? (
                filtered.map(o => (
                  <tr key={o._id} className="hover:bg-gray-50/80 transition-colors group">
                    <td className="px-5 py-4 font-mono text-xs font-bold text-primary-700">#{o.orderNumber}</td>
                    <td className="px-5 py-4">
                      <p className="font-bold text-gray-900">{o.shippingAddress.name}</p>
                      <p className="text-xs text-gray-400">{o.shippingAddress.phone}</p>
                    </td>
                    <td className="px-5 py-4 font-black text-gray-900 whitespace-nowrap">{o.total.toLocaleString('vi-VN')}₫</td>
                    <td className="px-5 py-4 text-gray-500 whitespace-nowrap">
                      {new Date(o.createdAt).toLocaleDateString('vi-VN')}
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-xs font-medium text-gray-600 uppercase bg-gray-100 px-2 py-1 rounded-md">{o.paymentMethod}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full whitespace-nowrap shadow-sm ${STATUS_MAP[o.orderStatus]?.color}`}>
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-12 text-center text-gray-400">Không tìm thấy đơn hàng nào</td>
                </tr>
              )}
            </tbody>
          </table>
