...
import { ShoppingCart, Heart, Eye, Star, ShoppingBag, ArrowRight, Crown } from 'lucide-react';
import { useCategories, getCategoryImg } from '../hooks/useCategories';
import { useBrands, getBrandLogo } from '../hooks/useBrands';
import { useAuth } from '../context/AuthContext';
import { useCart } from '../context/CartContext';
import { toast } from 'react-toastify';
const DEFAULT_CAT_IMG = 'https://placehold.co/80x80/f3f4f6/9ca3af?text=No+Image';
const DEFAULT_BRAND_IMG = 'https://placehold.co/100x50/f3f4f6/9ca3af?text=No+Logo';
const DEFAULT_PRODUCT_IMG = 'https://placehold.co/250x250/f3f4f6/9ca3af?text=Product';
...
const Home = () => {
  ...
  useEffect(() => {
    api.get('/products', { params: { isFeatured: true, status: 'active', limit: 8 } })
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 relative h-[300px] md:h-[400px] rounded-xl overflow-hidden shadow-md group">
            <img 
              src={settings?.banner1 ? `${BACKEND}${settings.banner1}` : "https://placehold.co/800x400/e50027/ffffff?text=SUMMER+SALE+Up+To+50%25"} 
              alt="Main Banner" 
              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-8">
              <h2 className="text-white text-3xl font-bold mb-2">
                {settings?.banner1Title || (settings?.banner1 ? "" : "Đại Tiệc Mùa Hè")}
              </h2>
              <p className="text-white text-lg mb-4">
                {settings?.banner1Sub || (settings?.banner1 ? "" : "Giảm giá lên đến 50% tất cả các mặt hàng")}
              </p>
              <Link to="/products" className="btn-primary w-max block">Mua Ngay</Link>
            </div>
          </div>
          <div className="flex flex-col gap-4">
            <div className="h-[142px] md:h-[192px] rounded-xl overflow-hidden shadow-md relative group">
               <img src={settings?.banner2 ? `${BACKEND}${settings.banner2}` : "https://placehold.co/400x200/333333/ffffff?text=GAMING+LAPTOP"} alt="Sub Banner 1" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"/>
               <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors flex flex-col justify-end p-4">
                  <h3 className="text-white font-bold text-sm">{settings?.banner2Title}</h3>
                  <p className="text-white/80 text-[10px] line-clamp-1">{settings?.banner2Sub}</p>
               </div>
            </div>
            <div className="h-[142px] md:h-[192px] rounded-xl overflow-hidden shadow-md relative group">
               <img src={settings?.banner3 ? `${BACKEND}${settings.banner3}` : "https://placehold.co/400x200/111111/ffffff?text=PC+BUILD"} alt="Sub Banner 2" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"/>
               <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors flex flex-col justify-end p-4">
                  <h3 className="text-white font-bold text-sm">{settings?.banner3Title}</h3>
                  <p className="text-white/80 text-[10px] line-clamp-1">{settings?.banner3Sub}</p>
               </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-8">
        ...
        {catLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white p-4 rounded-xl shadow-sm animate-pulse flex flex-col items-center gap-2">
              ...
              >
                <div className="mb-3 w-16 h-16 rounded-full overflow-hidden flex items-center justify-center group-hover:-translate-y-2 transition-transform shadow-sm border border-gray-100 bg-gray-50 text-3xl">
                  <img 
                    ...
                    onError={e => { (e.target as HTMLImageElement).src = DEFAULT_CAT_IMG; }}
                  />
                </div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-8">
         <div className="flex justify-between items-end mb-6">
            ...
            <Link to="/products" className="text-primary-600 hover:text-primary-800 font-medium text-sm flex items-center gap-1 group">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {loadingProducts ? (
              [...Array(4)].map((_, i) => (
                <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 h-[380px] animate-pulse flex flex-col">
              ...
              <p className="text-gray-500 col-span-full py-8 text-center bg-white rounded-xl border border-gray-100">Chưa có sản phẩm nổi bật nào.</p>
            ) : featuredProducts.map((product) => (
               <div key={product._id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-lg transition-shadow border border-gray-100 group relative flex flex-col">
                  ...
                  <button 
                     ...
                     title="Yêu thích">
                     <Heart className={`w-5 h-5 ${isProductInWishlist(product._id) ? 'fill-red-500' : ''}`} />
                  </button>
                  ...
                  <div className="p-4 border-t border-gray-50 flex-grow flex flex-col relative">
                     <Link to={`/product/${product._id}`} className="block">
                       <h3 className="font-medium text-gray-900 mb-1 line-clamp-2 hover:text-primary-600 transition-colors leading-snug min-h-[44px]" title={product.name}>
                         {product.name}
                       </h3>
                     </Link>
                     ...
                     <p className="text-xs text-gray-500 line-clamp-2 mb-2 h-[32px]">
                        {product.shortDesc || product.category?.name || 'Đang cập nhật'}
                     </p>
                     ...
                     <div className="flex items-center gap-3 text-[11px] text-gray-500 mb-3">
                        ...
                        <div className="flex items-center gap-1 min-w-0 truncate">
                           ...
                           <span className="truncate">Đã bán {product.soldCount || 0}</span>
                        </div>
                     </div>
                     ...
                     <div className="mt-auto flex items-end justify-between">
                        <div className="flex flex-col">
                           ...
                           {(product.comparePrice > product.price) && (
                             <span className="text-gray-400 text-sm line-through decoration-gray-300">{(product.comparePrice).toLocaleString('vi-VN')}₫</span>
                           )}
                        </div>
                        <button
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-4 justify-center md:justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          {brandLoading ? (
            [...Array(8)].map((_, i) => (
              <div key={i} className="w-24 h-12 bg-gray-100 rounded animate-pulse" />
            ))
          ) : brands.length === 0 ? (
            <p className="text-gray-400 text-sm w-full text-center py-4">Chưa có thương hiệu nào</p>
          ) : brands.map(brand => (
            <Link
              key={brand._id}
              ...
              title={brand.name}
            >
              <img src={getBrandLogo(brand.logo) || DEFAULT_BRAND_IMG} alt={brand.name}
                className="w-full h-full object-contain p-2"
                onError={e => { (e.target as HTMLImageElement).src = DEFAULT_BRAND_IMG; }}
              />
            </Link>
          ))}
        </div>
