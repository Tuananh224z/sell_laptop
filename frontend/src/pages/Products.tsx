    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex gap-4 p-4 hover:shadow-md transition-shadow group relative">
        <div className="relative shrink-0 w-32 h-28 bg-white flex items-center justify-center rounded-xl border border-gray-100 overflow-hidden">
          {discount > 0 && (
            <span className="absolute top-1 left-1 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full z-10">
              -{discount}%
            </span>
          )}
          <img
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src =
                "https://placehold.co/120x90/f3f4f6/9ca3af?text=Product";
            }}
          />
        </div>
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          <div>
            <Link to={`/product/${p._id}`}>
              <h3 className="font-semibold text-gray-900 hover:text-primary-600 transition-colors line-clamp-2 text-sm">
                {p.name}
              </h3>
            </Link>
          ...
          <div className="flex items-end justify-between mt-2">
            <div className="flex flex-col">
              <span className="text-primary-600 font-bold text-base leading-tight">
            ...
            <button
              ...
              className="bg-primary-50 text-primary-600 hover:bg-primary-600 hover:text-white disabled:opacity-40 p-2.5 rounded-xl transition-colors shadow-sm border border-primary-100"
            >
      ...
      <div className="p-4 border-t border-gray-50 flex-grow flex flex-col">
        ...
        <div className="flex items-center gap-3 text-[11px] text-gray-500 mb-3">
          ...
          <div className="flex items-center gap-1 truncate">
            ...
            <span className="truncate">Đã bán {p.soldCount || 0}</span>
          </div>
        </div>
        ...
        <div className="mt-auto flex items-end justify-between">
          ...
          <button
            ...
            onClick={(e) => onAddToCart(p, e)}
            title="Thêm vào giỏ hàng"
            className="bg-primary-50 text-primary-600 hover:bg-primary-600 hover:text-white disabled:opacity-40 p-2.5 rounded-xl transition-colors self-end shadow-sm border border-primary-100"
          >
            <ShoppingCart className="w-5 h-5" />
          </button>
        </div>
...
const Products = () => {
  ...
  const [selectedBrands, setSelBrands] = useState<string[]>(
  ...
  useEffect(() => {
    categoryService.getAll()
      .then((r: any) => setCategories(r.data.categories || r.data))
      .catch(() => {});
    brandService.getAll()
      .then((r: any) => setBrandList(r.data.brands || r.data))
      .catch(() => {});
  }, []);
  ...
  const isWished = (productId: string) =>
    user?.wishlist?.some(
      (w) => (typeof w === "string" ? w : (w as any)._id) === productId,
    ) ?? false;
  ...
  const activeFilterCount =
    ...
    (priceMin > 0 || priceMax < 999999999 ? 1 : 0);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className="text-xs text-primary-600 hover:underline pl-3"
          >
            Xoá khoảng giá
          </button>
        </div>
      ...
      {activeFilterCount > 0 && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 text-sm text-red-500 hover:text-red-600 border border-red-200 hover:border-red-300 rounded-xl py-2 transition-colors"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3 mb-5 flex-wrap">
              ...
              <button
                ...
                className="lg:hidden flex items-center gap-1.5 text-sm font-medium text-gray-700 border border-gray-200 rounded-xl px-3 py-2"
              >
              ...
              <div className="flex border border-gray-200 rounded-xl overflow-hidden">
                <button
                  ...
                  className={`p-2 transition-colors ${!listView ? "bg-primary-600 text-white" : "text-gray-400 hover:bg-gray-50"}`}
                >
            ...
            {loadingProducts ? (
              ...
              >
                {[...Array(perPage)].map((_, i) => (
                  <div
                    key={i}
                    className="bg-white rounded-xl border border-gray-100 animate-pulse h-72"
                  />
                ))}
              </div>
            ) : products.length > 0 ? (
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                  (n) => (
                    <button
                      key={n}
                      onClick={() => setPage(n)}
                      className={`w-9 h-9 rounded-xl border text-sm font-semibold transition-colors ${n === page ? "bg-primary-600 text-white border-primary-600" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                    >
                      {n}
                    </button>
                  ),
      ...
      {showFilter && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-72 bg-white h-full overflow-y-auto p-5 shadow-xl">
            ...
            <button
              onClick={() => setShowFilter(false)}
              className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-3 rounded-xl mt-6 transition-colors"
            >
              Áp dụng
            </button>
          </div>
