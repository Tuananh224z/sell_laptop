        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-primary-500 outline-none"
            >
            ...
            <div className="flex gap-2">
              {["low", "medium", "high"].map((p) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all ${form.priority === p ? "bg-primary-50 border-primary-500 text-primary-700" : "bg-white border-gray-100 text-gray-500"}`}
                >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Vui lòng mô tả chi tiết vấn đề, bao gồm mã đơn hàng nếu có..."
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden min-h-[600px] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-start gap-4">
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-primary-600 transition-colors shrink-0 mt-0.5 font-medium group"
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span
                className={`inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full ${cfg.color} uppercase`}
              >
              ...
              <span className="text-xs text-gray-400">
                Tạo lúc {new Date(detail.createdAt).toLocaleDateString("vi-VN")}
              </span>
            </div>
          </div>
        </div>
        ...
        <div
          ref={chatRef}
          className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/40 custom-scrollbar"
        >
          {/* Initial content */}
          <div className="flex justify-end">
            <div className="max-w-[85%] flex flex-col items-end">
              <p className="text-[10px] font-bold text-gray-400 mb-1.5 uppercase">
                Tôi
              </p>
              <div className="bg-primary-600 text-white rounded-2xl rounded-tr-sm px-4 py-3 shadow-sm shadow-primary-100">
                ...
                <p className="text-[10px] mt-2 opacity-70 font-bold">
                  {new Date(detail.createdAt).toLocaleTimeString("vi-VN", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </p>
          ...
          {detail.messages.slice(1).map((msg, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <p className="text-[10px] font-bold text-gray-400 mb-1.5 uppercase">
                    {isMe ? "Tôi" : "CSKH TechStore"}
                  </p>
                  <div
                    className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-primary-600 text-white rounded-tr-sm shadow-sm shadow-primary-100" : "bg-white border border-gray-100 text-gray-800 rounded-tl-sm shadow-sm"}`}
                  >
                    {msg.mediaUrl && (
                      <div className="mb-2 max-w-xs overflow-hidden rounded-lg cursor-pointer">
                        {msg.mediaType === "video" ? (
                          ...
                          <img
                            ...
                            onClick={() =>
                              window.open(`${BACKEND}${msg.mediaUrl}`, "_blank")
                            }
                          />
                        )}
                      </div>
          ...
          {detail.status === "resolved" && (
            <div className="flex justify-center">
              <div className="inline-flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-xs font-bold px-4 py-2 rounded-full uppercase tracking-tight">
                <CheckCircle className="w-3.5 h-3.5" /> Yêu cầu đã được giải
                quyết
              </div>
            </div>
          )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Gửi yêu cầu hỗ trợ và trao đổi trực tiếp với nhân viên TechStore
          </p>
        </div>
        <button
          onClick={() => setShowNew(true)}
          className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-bold px-5 py-2.5 rounded-xl transition-all text-sm shadow-md shadow-primary-100 active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            icon: Mail,
            title: "Email hỗ trợ",
            desc: settings?.email || "hotro@techstore.vn",
          ...
          <div
            key={card.title}
            className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4 hover:border-primary-200 hover:shadow-md transition-all cursor-pointer group"
          >
            <div
            ...
            <div className="min-w-0">
              ...
              <p
                className="text-sm text-primary-600 font-extrabold mt-0.5 truncate"
                title={card.desc}
              >
                {card.desc}
              </p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-300">
            ...
            <button
              onClick={() => setShowNew(true)}
              className="mt-4 text-primary-600 hover:text-primary-700 text-sm font-extrabold underline underline-offset-4"
            >
              Tạo yêu cầu ngay
            </button>
          </div>
            ...
            {tickets.map((ticket) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-6 py-5 hover:bg-gray-50/70 transition-all flex items-center gap-4 group"
                >
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${cfg.color.replace("text-", "bg-").replace("700", "50")} ${cfg.color}`}
                  >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-tighter ${cfg.color} ${cfg.color.replace("text-", "bg-").replace("700", "100")}`}
                    >
                      {cfg.label}
                    </span>
